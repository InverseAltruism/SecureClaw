...
# SecureClaw Linux connection helper.
# Run on a Linux PC to open an SSH tunnel to a SecureClaw VPS.

readonly SCRIPT_NAME="secureclaw-connect-linux"
...
usage() {
    cat <<'EOF'
Usage:
  bash connect-openclaw-linux.sh [options]
...
interactive_prompts() {
    echo
    info "SecureClaw Linux connection wizard"
    echo
